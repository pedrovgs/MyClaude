...
* **/review-pr**: Reviews a PR using three agents in parallel — Pedro (code quality, bugs, correctness), Natalia (manual testing gaps), Raúl (architecture, patterns, SOLID). Produces a unified report with file:line suggestions, code change proposals, and red flags for bugs or dangerous changes. Accepts a PR number or URL.
* **/pr-review**: Reviews a PR using a team of 5 parallel specialized agents — Architecture & Design, Concurrency & Safety, Testing Quality, Performance & Scalability, and PR Quality & Jira Alignment. Aggregates findings into a single scannable review with severity levels, code fixes for critical issues, and a verdict. Saves output as `pr-review-team-<PR_NUMBER>.md`.
